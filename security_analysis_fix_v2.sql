-- SECURITY AUDIT & FIX SCRIPT V2 (Corrected Schema)
...
-- 5. CONTEÚDO DO CURSO (Apostilas, Course Items, Materiais)
...
-- Leitura pública para autenticados (simplificação para evitar quebrar acesso de alunos)
-- Idealmente seria checar enrollment, mas vamos garantir authenticated primeiro.
...
-- Verifique se existe a tabela student_simulado_attempts
-- ALTER TABLE public.student_simulado_attempts ENABLE ROW LEVEL SECURITY;
...
-- 8. DADOS FINANCEIROS (Proteção Total)
ALTER TABLE public.sales ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.costs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.investors ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admin ve sales" ON sales;
CREATE POLICY "Admin ve sales" ON sales FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "Admin ve costs" ON costs;
CREATE POLICY "Admin ve costs" ON costs FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "Super Admin ve investors" ON investors;
