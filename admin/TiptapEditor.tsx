...
const TiptapEditor = React.forwardRef<TiptapRef, TiptapEditorProps>(({ content, onChange, placeholder, minHeight = '400px', uploadPath = '' }, ref) => {
    ...
    return (
        <div className="border border-slate-200 rounded-[32px] overflow-hidden bg-white shadow-inner relative">
            {/* Main Toolbar - kept for full access */}
            <div className="flex flex-wrap items-center gap-1 p-4 bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                ...
                <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 4 }).run()} isActive={editor.isActive('heading', { level: 4 })} icon="format_h4" label="Título 4" />
                ...
                <MenuButton onClick={() => editor.chain().focus().insertContent('<p>[--EXEMPLO--] Exemplo... [/--EXEMPLO--]</p>').run()} icon="lightbulb" label="Inserir Exemplo" />
                <MenuButton onClick={() => editor.chain().focus().insertContent('<p>[--IMPORTANTE--] Importante... [/--IMPORTANTE--]</p>').run()} icon="priority_high" label="Inserir Importante" />

                <div className="w-px h-6 bg-slate-200 mx-2" />

                {/* Special BPA Tags - Restored */}
            ...
            {/* Bubble Menu for Formatting (Selection) */}
            {editor && (
                <BubbleMenu editor={editor} className="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                    <div className="flex p-1 gap-1">
                        ...
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} isActive={editor.isActive('heading', { level: 1 })} icon="format_h1" label="H1" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} isActive={editor.isActive('heading', { level: 2 })} icon="format_h2" label="H2" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} isActive={editor.isActive('heading', { level: 3 })} icon="format_h3" label="H3" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 4 }).run()} isActive={editor.isActive('heading', { level: 4 })} icon="format_h4" label="H4" />
                        <div className="w-px h-6 bg-slate-200 mx-1 self-center" />
                        <MenuButton onClick={() => editor.chain().focus().setTextAlign('left').run()} isActive={editor.isActive({ textAlign: 'left' })} icon="format_align_left" label="Left" />
                        <MenuButton onClick={() => editor.chain().focus().setTextAlign('center').run()} isActive={editor.isActive({ textAlign: 'center' })} icon="format_align_center" label="Center" />
                        <MenuButton onClick={() => editor.chain().focus().setTextAlign('right').run()} isActive={editor.isActive({ textAlign: 'right' })} icon="format_align_right" label="Right" />
                    </div>
                </BubbleMenu>
            )}

            {/* Floating Menu for Blocks (Empty Line) */}
            {editor && (
                <FloatingMenu editor={editor} className="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                    <div className="flex p-1 gap-1">
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} isActive={editor.isActive('heading', { level: 1 })} icon="format_h1" label="Título 1" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} isActive={editor.isActive('heading', { level: 2 })} icon="format_h2" label="Título 2" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} isActive={editor.isActive('heading', { level: 3 })} icon="format_h3" label="Título 3" />
                        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 4 }).run()} isActive={editor.isActive('heading', { level: 4 })} icon="format_h4" label="Título 4" />
                        <MenuButton onClick={() => editor.chain().focus().toggleBulletList().run()} isActive={editor.isActive('bulletList')} icon="format_list_bulleted" label="Lista" />
                        <MenuButton onClick={() => editor.chain().focus().toggleOrderedList().run()} isActive={editor.isActive('orderedList')} icon="format_list_numbered" label="Numerada" />
                        <div className="w-px h-6 bg-slate-200 mx-1 self-center" />
                        <button
                            onClick={(e) => {
                                e.preventDefault();
                        ...
                        <MenuButton onClick={() => editor.chain().focus().insertContent('<p>[--EXEMPLO--] Exemplo... [/--EXEMPLO--]</p>').run()} icon="lightbulb" label="Inserir Exemplo" />
                        <MenuButton onClick={() => editor.chain().focus().insertContent('<p>[--IMPORTANTE--] Importante... [/--IMPORTANTE--]</p>').run()} icon="priority_high" label="Inserir Importante" />
                        <MenuButton onClick={() => editor.chain().focus().insertContent('<p>[--LEI--] Lei... [/--LEI--]</p>').run()} icon="gavel" label="Inserir Lei" />
                    </div>
                </FloatingMenu>
            )}
