...
const CourseView: React.FC = () => {
  ...
  // Group standard apostilas (excluding resolutions AND resumos)
  const groupedItems = useMemo(() => items.filter(i => !i.apostila?.is_resolution_notebook && !i.apostila?.is_resumo_8020).reduce((acc: any, curr) => {
  ...
  // Group summary (resumos) apostilas
  const groupedResumos = useMemo(() => items.filter(i => !i.apostila?.is_resolution_notebook && i.apostila?.is_resumo_8020).reduce((acc: any, curr) => {
    const disciplineName = curr.apostila?.disciplina?.name || 'Geral';
    if (!acc[disciplineName]) acc[disciplineName] = [];
    acc[disciplineName].push(curr);
    return acc;
  ...
  useEffect(() => {
    const disciplineKeys = [
      ...
      ...Object.keys(groupedResumos).map(k => k + '_resumo')
    ];
            ...
            {(activeTab === 'apostilas' || activeTab === 'resolucoes') && (
              <div className="space-y-10">
                ...
                {(() => {
                  ...
                  let currentResumos = activeTab === 'apostilas' ? groupedResumos : {};
                  ...
                  if (searchTerm) {
                    ...
                    const filteredResumos: any = {};
                    Object.entries(currentResumos).forEach(([discipline, items]: [string, any]) => {
                      const matchingItems = items.filter((item: any) => 
                        item.apostila?.title?.toLowerCase().includes(searchTerm.toLowerCase())
                      );
                      if (matchingItems.length > 0) {
                        filteredResumos[discipline] = matchingItems;
                      }
                    });
                    currentResumos = filteredResumos;
                  }

                  const hasNoItems = Object.keys(currentGroups).length === 0 && Object.keys(currentResumos).length === 0;
                  ...
                  return (
                    <div key={discipline} className="space-y-4 bg-white/50 rounded-[40px] border border-slate-100 p-2 overflow-hidden shadow-sm">
                      ...
                      {activeTab === 'apostilas' && Object.keys(currentResumos).length > 0 && (
                        <div className="mt-12 space-y-6">
                          <div className="flex items-center justify-between px-4">
                            <div className="flex items-center gap-3">
                              <div className="size-8 rounded-lg bg-emerald-500 flex items-center justify-center text-slate-950 shadow-lg shadow-emerald-500/20 animate-pulse">
                                <span className="material-symbols-outlined text-base font-black">auto_awesome</span>
                              </div>
                              <div>
                                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Resumos Rápidos 80/20</h3>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">O essencial focado em resultados</p>
                              </div>
                            </div>
                            <div className="h-px flex-1 bg-slate-100 mx-6 hidden sm:block"></div>
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 border border-slate-100 px-3 py-1 rounded-full">
                              {Object.values(currentResumos).reduce((acc: number, curr: any) => acc + curr.length, 0)} Materiais
                            </span>
                          </div>

                          <div className="space-y-4">
                            {Object.entries(currentResumos).map(([discipline, disciplineItems]: [string, any]) => {
                              const isCollapsed = collapsedDisciplines.includes(discipline + '_resumo');
                              return (
                                <div key={discipline + '_resumo'} className="space-y-4 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 rounded-[40px] border border-slate-800/80 p-2 overflow-hidden shadow-2xl shadow-slate-950/20 transition-all duration-300">
                                  <button
                                    onClick={() => setCollapsedDisciplines(prev => isCollapsed ? prev.filter(d => d !== discipline + '_resumo') : [...prev, discipline + '_resumo'])}
                                    className="w-full flex items-center justify-between p-6 hover:bg-slate-800/40 transition-all rounded-[32px]"
                                  >
                                    <div className="flex items-center gap-4">
                                      <div className={`size-10 rounded-xl flex items-center justify-center ${isCollapsed ? 'bg-slate-800 text-slate-400' : 'bg-emerald-500 text-slate-950 shadow-lg shadow-emerald-500/20'}`}>
                                        <span className="material-symbols-outlined text-xl">{isCollapsed ? 'folder' : 'folder_open'}</span>
                                      </div>
                                      <h3 className="text-sm font-black text-slate-100 uppercase tracking-widest text-left">{discipline}</h3>
                                    </div>
                                    <div className="flex items-center gap-4">
                                      <span className="text-[9px] font-black text-slate-400 uppercase">{disciplineItems.length} Resumos</span>
                                      <span className={`material-symbols-outlined text-slate-400 transition-transform duration-500 ${isCollapsed ? '' : 'rotate-180'}`}>expand_more</span>
                                    </div>
                                  </button>
                                      ...
                                      {disciplineItems.map((item: any) => {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <div className="flex-1">
                                              <p className={`font-bold text-xs uppercase tracking-tight ${isRead ? 'text-emerald-400' : 'text-slate-100 group-hover:text-emerald-300 transition-colors'}`}>
                                                {item.apostila?.title}
                                              </p>
                                              <span className="mt-1 px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[7px] font-black rounded uppercase tracking-widest w-fit block animate-pulse">
                                                Resumo 80/20
                                              </span>
                                            </div>
